...
def load_model(model_path, data_path='data/coco.yaml', device_type='cpu'):
    ...
    if device_type != 'cpu': # half precision only supported on CUDA
        model.half()  # to FP16
        model.norm_val = model.norm_val.half() # set to FP16 if not on CPU
        print("Model set to FP16")
     
    
    return model, device


def prediction(source, model, device, mp_q1, img_size=640, n_th_frame=1, save_img=False, output_img='output/detect.jpg', use_colab=False, conf_thres=0.25):
    """
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #print("----------- Already continue in main program")
        ########################### CALCULATION OF FEATURE MAPS ################################
        """
        if num_iter in (0,1,2,3):
            path = "tensor_collection_test_" + str(num_iter) + ".pkl"
            s = time.time()
            save_pickle(path, model.tensor_collection)
            e = time.time()
            print(f"Collection saved in {(e-s) * 1000} ms")
        """
        """
        if mp_q1.empty() and (num_iter % n_th_frame) == 0:
            #mp_q1.put(model.tensor_collection, block=False) # place feature map dictionary in MP queue, only if queue empty
            #mp_q1.put(model.tensor_list, block=False)
            #mp_q1.join()
            pass
...
if __name__ == '__main__':
    """
    ******* TODO ********
    - add single image processing (no MP, one prediction)
    - find better way to stop program
    """
    ...
    parser.add_argument('-n', '--numproc', dest='num_proc',
                    type=int, default=4,
                    help='number of feature map calculation')
    ...
    #mp_q_in = mgr.Queue()
    #mp_q_out = mgr.Queue()
    #mp_q_in = mgr.JoinableQueue()
    #mp_q_out = mgr.JoinableQueue()
    ...
    print("... for Display ...")
    p = mp.Process(target=DisplayFeatureMaps.display_results, args=(mp_q_out,num_proc))
    proc_list.append(p)
    p.start()
    ...
    with torch.no_grad():
        ret = prediction(args.source, model, device, mp_q_in, 
    ...
    if args.device_type == 'gpu':
        torch.cuda.empty_cache() # empty whole graphics memory
