...
def prediction(source, model, device, mp_q_in, threading_queue=None, event=None, event_terminating=None, img_size=640, n_th_frame=1, save_img=False, output_img='output/detect.jpg', use_colab=False, conf_thres=0.25):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # feature maps ready for postprocessing
        if not event.is_set() and (num_iter % n_th_frame) == 0: # only place if thread has successfully placed object and cleared event
            threading_queue.put(model.tensor_collection, block=False)
        ...
        if threading_queue.empty():
            ...
            pass
        e = time.time()
        print(f"Threading Queue Handling Time: {(e - s) * 1000} ms")
        """
        """
        s = time.time()
        if mp_q_in.empty() and (num_iter % n_th_frame) == 0:
...
if __name__ == '__main__':
    ...
    ******* TODO ********
    ...
    for i in range(num_proc):
        ...
        p.start()
    print("... for Display ...")
    p = mp.Process(target=display_results, args=(mp_q_out, num_proc, event_terminating))
    proc_list.append(p)
    p.start()
