...
def prediction(source, model, device, mp_q_in, threading_queue=None, event=None, event_terminating=None, img_size=640, n_th_frame=1, save_img=False, output_img='output/detect.jpg', use_colab=False, conf_thres=0.25):
    """
    TESTING, NOT FINISHED YET
    ONLY RETURNS ONE TENSOR COLLECTION FOR TESTING OF VEDO RENDERING
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if not event.is_set() and (num_iter % n_th_frame) == 0:
            threading_queue.put(model.tensor_collection, block=False)
            #print("Output placed in threading queue")
            event.set() # signal to queue thread to handle queue IO
        else:
...
def queue_handler(threading_queue, mp_queue_in, event):
    """
...
def worker_func(queue_in, queue_out, worker_id, event_terminating, metadata_display, event_display_sync, lock_display_sync):
    """
...
if __name__ == '__main__':
    ...
    ******* TODO ********
    ...
    mp_q_in = mgr.Queue()
    mp_q_out = mgr.Queue()
    event = threading.Event() # synchronization of queue handler thread
    event_terminating = mp.Event() # used to securely stop child processes
    
    # display process
    buff_display_name = "display"
    shm = mp.shared_memory.SharedMemory(create=True, size=5000000, name=buff_display_name)
    ...
    # queue handler QH
    thread_QH = threading.Thread(target=queue_handler, args=(threading_queue, mp_q_in, event), daemon=True)
    ...
    for i in range(num_proc):
        p = mp.Process(target=worker_func, args=(mp_q_in, mp_q_out, i, event_terminating, metadata_display,
    ...
    with torch.no_grad():
        ret = prediction(args.source, model, device, mp_q_in, threading_queue=threading_queue, 
                        event=event, event_terminating=event_terminating,
