...
def prediction(source, model, device, mp_q_in, threading_queue=None, event_handler_thread=None, event_terminating=None, img_size=640, n_th_frame=1, 
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if not event_handler_thread.is_set() and (num_iter % n_th_frame) == 0:
            ...
            #time.sleep(1/1000) # synchronization time to retrieve feature map data, not needed because of thread state check in line 282
...
if __name__ == '__main__':
    ...
    ******* TODO ********
    ...
    model, device = load_model('yolov7.pt', device_type=args.device_type, save_init_tensor_collection=False)
    print("Model loaded.")
